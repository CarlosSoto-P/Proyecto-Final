...
$insert_megusta=("INSERT INTO  megustas (idUsuario,idPublicacion) VALUES ($usuario,$post)");


$res_inserMegusta = mysqli_query($connLocalhost,$insert_megusta) or trigger_error("fallo megusta");

$update_publicacion = ("UPDATE publicacion SET megustas = megustas+1 WHERE idPublicacion = $post");
$res_update = mysqli_query($connLocalhost,$update_publicacion);



}else{
$delete_megusta = ("DELETE FROM megustas WHERE idPublicacion =$post AND idUsuario =$usuario");
...
$contar = ("SELECT * FROM publicacion WHERE idPublicacion = $post");
